...
public class Transform implements RequestHandler<Request, HashMap<String, Object>>, HttpFunction {
    ...
    public HashMap<String, Object> procedure(Request request, HttpRequest httpRequest, HttpResponse httpResponse) throws IOException {
        ...
        try {
            InputStream inputStream = readBlobContent(blobStore, containerName, objectName);
            String content = transform(inputStream);
            writeBlob(blobStore, containerName, "Transform_"+objectName, content);
        } catch (Exception e) {
            e.printStackTrace();
            connect = false;
